      ...
      <div className="border border-border bg-background shadow-[8px_8px_0_0_rgba(234,88,12,0.2)] p-6 md:p-8 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 items-center relative z-10">
          ...
          <div className="flex flex-col items-center md:items-start gap-2">
            <span className="font-mono text-[10px] uppercase tracking-[2px] text-muted-foreground flex items-center gap-1.5 cursor-help">
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-1.5 hover:text-accent transition-colors"><TreePine className="h-3 w-3 text-accent" /> // RINGS_DETECTED <Info className="h-3 w-3 opacity-50" /></div>
                </TooltipTrigger>
                <TooltipContent className="max-w-[200px] border border-accent/20 bg-background text-foreground font-mono text-[10px]">
                  The absolute number of valid annual growth boundaries identified from pith to bark.
                </TooltipContent>
              </Tooltip>
            </span>
            ...
            <span className="font-mono text-xs text-muted-foreground uppercase tracking-[1px] mt-1 flex items-center gap-1 cursor-help">
              <Tooltip>
                <TooltipTrigger className="hover:text-foreground transition-colors">
                  EST. AGE: ~{result.estimated_age} YRS <Info className="h-3 w-3 inline opacity-50" />
                </TooltipTrigger>
                <TooltipContent className="max-w-[200px] border border-accent/20 bg-background text-foreground font-mono text-[10px]">
                  The chronological age estimate derived directly from the AI&#39;s ring count.
                </TooltipContent>
              </Tooltip>
            </span>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className={cn(
                  "mt-2 inline-flex items-center gap-1.5 font-mono text-[10px] font-bold px-3 py-1 border uppercase tracking-[1px] cursor-help transition-all hover:opacity-80",
              ...
              <TooltipContent className="max-w-[250px] border border-accent/20 bg-background text-foreground font-mono text-[10px]">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <Tooltip key={label}>
                <TooltipTrigger asChild>
                  <div className="flex flex-col gap-1 border border-border/50 bg-surface/50 p-3 hover:border-accent/40 hover:bg-surface/80 transition-all cursor-help">
                ...
                <TooltipContent className="max-w-[200px] border border-accent/20 bg-background text-foreground font-mono text-[10px] !z-50 leading-relaxed">
                  {tip}
                </TooltipContent>
              </Tooltip>
            ))}
            ...
            {result.trend && (
              <Tooltip>
                ...
                <TooltipContent className="max-w-[200px] border border-accent/20 bg-background text-foreground font-mono text-[10px] !z-50 leading-relaxed">
                  The overall trajectory of sequential ring widths. Increasing indicates accelerating growth; decreasing typically indicates age maturation or encroaching environmental stress.
                </TooltipContent>
              </Tooltip>
            )}
          </div>
